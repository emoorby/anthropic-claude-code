...
input string   _sep_dayfilter        = ""; // ════════════ DAY OF WEEK FILTER ════════════
...
//+------------------------------------------------------------------+
//| Helper: Check if day-of-week filter passes                        |
//+------------------------------------------------------------------+
bool CheckDayFilter()
{
   if(!EnableDayFilter)
   ...
   {
      case 1: if(!TradeMonday)    { Log("Day filter BLOCKED: Monday");    return false; } break;
      case 2: if(!TradeTuesday)   { Log("Day filter BLOCKED: Tuesday");   return false; } break;
      case 3: if(!TradeWednesday) { Log("Day filter BLOCKED: Wednesday"); return false; } break;
      case 4: if(!TradeThursday)  { Log("Day filter BLOCKED: Thursday");  return false; } break;
      case 5: if(!TradeFriday)    { Log("Day filter BLOCKED: Friday");    return false; } break;
...
int PlaceSellStop(double value5)
{
   ...
   if(!CheckDayFilter())
   {
      LogTradeRejected(false, "ZZSemafor", "DAY_FILTER", TimeCurrent(), value5,
                       PipsToRisk, atrPips, erValue, 0.0, "M30");
      return -1;
   }
...
int PlaceBuyStop(double value6)
{
   ...
   if(!CheckDayFilter())
   {
      LogTradeRejected(true, "ZZSemafor", "DAY_FILTER", TimeCurrent(), value6,
                       PipsToRisk, atrPips, erValue, 0.0, "M30");
      return -1;
   }
                               ...
                               double candleClose)
{
   ...
   if(!CheckDayFilter())
   {
      LogTradeRejected(isBuy, "ATRCandle", "DAY_FILTER", barTime, candleClose,
                       candleSizePips, atrPips, erValue, candleSizePips, tfStr);
      return false;
   }
...
int OnInit()
{
   Log("=== HiLo Breakout EA v2.11 Starting ===");
   ...
   if(EnableDayFilter)
   {
      Log(StringFormat("Day Filter: ON | Mon=%s Tue=%s Wed=%s Thu=%s Fri=%s",
          (TradeMonday ? "YES" : "NO"), (TradeTuesday ? "YES" : "NO"),
          (TradeWednesday ? "YES" : "NO"), (TradeThursday ? "YES" : "NO"),
          (TradeFriday ? "YES" : "NO")));
   }
   else
      Log("Day Filter: OFF");
